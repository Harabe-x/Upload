                            ...
                            <li class="mb-2 font-semibold text-xl">
                                <NavigationBarSubMenu icon={menuItem.Icon} title={menuItem.Title}>
                                    {#each menuItem.SubItems as subItem }
                                        <NavigationMenuItem name={subItem.Title} iconSize={5} url={subItem.Url} icon={subItem.Icon}> {subItem.Title} </NavigationMenuItem>
                                    {/each}
                                </NavigationBarSubMenu>     
                            </li>  
                            {:else}
                                <NavigationMenuItem name={menuItem.Title} icon={menuItem.Icon} url={menuItem.Url} class="mb-2 font-semibold text-xl"> {menuItem.Title} </NavigationMenuItem>                
                        {/if}
                {/each}
            </ul>
</div>